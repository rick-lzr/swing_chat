...
public class ClientThread extends Thread {
		...
		try {
			while (DataBuffer.clientSeocket.isConnected()) {
				...
				if (type == ResponseType.LOGIN) {
					...
					ClientUtil.appendTxt2MsgListArea("【系统消息】用户"+newUser.getUsername() + "已进入聊天室！请遵守聊天室守则，文明聊天，友善交流。\n"); 
				}else if(type == ResponseType.LOGOUT){
					User newUser = (User)response.getData("logoutUser");
					Boolean boolean1 = DataBuffer.onlineUserListModel.removeElement(newUser);
					
					ChatFrame.onlineCountLbl.setText(
							"在线用户列表("+ DataBuffer.onlineUserListModel.getSize() +")");
					ClientUtil.appendTxt2MsgListArea("【系统消息】用户"+newUser.getUsername() + "下线了！\n");
					System.out.println(boolean1);
					System.out.println(newUser);
					System.out.println(DataBuffer.onlineUserListModel.getOnlineUsers());
					System.out.println(DataBuffer.onlineUserListModel.getSize());
				...
				}else if(type == ResponseType.LOGYOU){ //开始接收文件
					((ChatFrame) currentFrame).LogOutMe();
				}
			}
		} catch (IOException e) {
			//e.printStackTrace();
	...
	private void toSendFile(Response response) {
		...
		String fromName = sendFile.getFromUser().getUsername() 
				+ "(" + sendFile.getFromUser().getUserid() + ")";
